...
-- @withScope@ / @editScope@ set on the current thread. Our processor exports
-- synchronously on that same thread, so the harvest always observes the scope
-- active where the span ended.
--
...
withInMemoryTracer action = do
  ...
  tracer <- getTracer provider "sentry-optics-checkout" tracerOptions
  ...
  SpanProcessor
    { spanProcessorOnStart = \_ _ -> pure (),
      spanProcessorOnEnd = \immSpan -> do
        ...
        _ <- exporter.spanExporterExport (HashMap.singleton "sentry-optics-checkout" (Vector.singleton immSpan))
        pure (),
